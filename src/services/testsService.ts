...
export async function createTest(test) {
  ...
  const findTeachersDisciplinesId =
    await testsRepository.findTeachersDisciplinesId(
      parseInt(teacherId),
      parseInt(disciplineId)
    );
  if (!findTeachersDisciplinesId)
    throw {
      type: "not_found",
      message: "Discipline is not assigned to selected teacher",
    };
  const testData = {
    name,
    pdfUrl,
